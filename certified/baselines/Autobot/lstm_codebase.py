...
from baselines.Autobot.modules import Hidden2Normal, InputEmbedding

from baselines.Autobot import augmentation

from baselines.Autobot.utils import center_scene, inverse_scene

NAN = float('nan')


def drop_ped_with_missing_frame(xy): # xy: (21, n, 68)
    """
...
class LSTMPredictor(object):
    ...
    def __call__(self, scene, scene_goal, n_predict=12, modes=1, predict_all=True, obs_length=9, start_length=0, args=None):
        self.model.eval()
        ...
        with torch.no_grad():
            ...
            xy = scene.detach().cpu().numpy()
                        
            xy = drop_ped_with_missing_frame(xy)
            curr_scene = drop_distant(xy, max_num_peds=N)
            curr_scene, rotation, center, = center_scene(curr_scene)
            ...
            ego_in_return = torch.Tensor(ego_in_return).to('cuda')
            agents_in_return = torch.Tensor(agents_in_return).to('cuda')
            roads_return = torch.Tensor(roads_return).to('cuda')
            agent_types_return = torch.Tensor(agent_types_return).to('cuda')
            ...
            for num_p in range(modes):
                ...
                pred_obs = pred_obs[num_p,:, 0,:,:2].detach().cpu().numpy()
